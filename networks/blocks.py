...
class Upsampler(nn.Sequential):
    def __init__(self, conv, upscale_factor, num_features, norm_type=False, act_type=False, bias=True):

        m = []
        if (upscale_factor & (upscale_factor-1)) == 0:
            for i in range(int(math.log(upscale_factor, 2))):
                m.append(conv(num_features, 4*num_features, 3, bias))
            ...
            m.append(conv(num_features, 9*num_features, 3, bias))
...
class Downsampler(nn.Sequential):
    def __init__(self, conv, in_channel, out_channel, num_features):
        self.conv_1 = conv(in_channel, num_features, 1)
        self.conv_2 = conv(num_features, num_features, 3)
        self.conv_3 = conv(num_features, out_channel, 1)
