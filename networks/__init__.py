...
def define_net(opt):
    ...
    if which_model.find('SRFBN') >= 0:
        ...
        net = RCAN(
            num_features=opt['num_features'], n_resgroups=opt['n_resgroups'], n_resblocks=opt['n_resblocks'],
            kernel_size=3, reduction=opt['reduction'], upscale_factor=opt['scale']
        )

    elif which_model.find('GRAPH') >= 0:
        from .graph import Graph
        net = Graph(
            in_channels=opt['in_channels'], out_channels=opt['out_channels'],
            num_features=opt['num_features'], upscale_factor=opt['scale']
        )
    
    elif which_model.find('LAMBDANET') >= 0:
    ...
    elif which_model.find('GCNSR') >= 0:
        from .gcn import GCNSR
        net = GCNSR(
            in_channels=opt['in_channels'], out_channels=opt['out_channels'],
            num_features=opt['num_features'], upscale_factor=opt['scale']            
        )
